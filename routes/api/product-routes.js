...
try {
  ...
  !product ? res.status(404).json({ message: 'Cound not locate product by id' }) : res.status(200).json(product);
} catch (err) {
  res.status(500).json({ message: 'Could not find product by id' });
}
...
router.delete('/:id', async (req, res) => {
  ...
  try { 
    ...
    !deleted ? res.status(404).json({ message: 'Could not find product by id' }) : res.status(200).json(deleted);
  } catch (err) {
